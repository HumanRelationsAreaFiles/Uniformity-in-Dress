...
parameters{
  ...
  vector[N_f + 1] b_strat; // slope of latent factors on stratification
  vector[N_f + 1] b_records; // slope of latent factors on stratification
  ...
  vector[N_f + 1] b_RS; // slope of latent factors on RS
  ...
  vector[N_f + 1] b_int; // overall effect of political integration
  simplex[max(pol_int)] scale_int[N_f + 1]; //  monotonic intervals of commnuity size 
...
transformed parameters{
 ...
 eta_RS_v = eta_RS_z * sigma_eta_RS;
  ...
  for (n in 1:N) {
  for (f in 1:N_f + 1) {
    eta_v[n,f] = alpha_eta[f] + eta_v[n,f] + phy_v[n,f] + EP_v[n,f] + geo_v[n,f] + b_RS[f]*eta_RS_v[n] + b_strat[f]*strat[n] + b_records[f]*records[n] + b_comm[f]*mo(scale_comm[f],comm_size[n]) + b_int[f]*mo(scale_int[f],pol_int[n]);
  }
  }
...
for (f in 1:(N_f+1)) {
  ...
  scale_int[f] ~ dirichlet(rep_vector(2, max(pol_int))); 
}
